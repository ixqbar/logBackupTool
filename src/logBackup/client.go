...
func Transfer(server string, file string, path string, name string) error {
	...
	if _, err := io.Copy(h, f); err != nil {
		Debugf("Get file %s md5sum failed %v", file, err)
		return err
	}

	m := hex.EncodeToString(h.Sum(nil))
	Debugf("Get file %s md5sum %s", server, m)
	...
	for {
		...
		if err != nil {
			Debugf("Transfer target server %s with header failed %v", server, err)
			return err
		}
		n += nr
		if n >= headerLen {
	...
	if err != nil {
		Debugf("Transfer target server %s failed %v", server, err)
		return err
	}

	responseStr := strings.Trim(string(response[:l]), "\r\n")
	if l < 2 || responseStr != "OK" {
		Debugf("Transfer target server %s resposne \n",server, responseStr)
